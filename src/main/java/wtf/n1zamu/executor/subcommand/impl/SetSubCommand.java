...
public class SetSubCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        NCases.getInstance().getCrateDatabase().updateKeyAmount(playerName, caseName, amount);
        Map<String, String> placeholders = new HashMap<>();
        placeholders.put("%player%", playerName);
        placeholders.put("%amount%", String.valueOf(amount));
        placeholders.put("%case%", caseName);
        NCases.getInstance().getMessageManager().sendMessage((Player) sender, "setCase", placeholders);
